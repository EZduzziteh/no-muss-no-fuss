...
//  	define( 'SALON_FOR_REFACTOR', false );
// }
...
//_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/_/ デモ
	...
	private function _sl_daily__delete_temporary_sql () {
		...
		if (!empty($result['SALON_CONFIG_CONFIRM_STYLE']) && $result['SALON_CONFIG_CONFIRM_STYLE'] == CONFIRM_BY_ADMIN) return;
...
///		$help .= '<li><strong>'.__('Number of the Shops',SL_DOMAIN).'</strong> - '.__('Now plural shops select only.',SL_DOMAIN ) . '</li>';
		...
		$help .= '<li><strong>'.__('Operation Log Setting',SL_DOMAIN).'</strong> - '.__('Check the mark "Operation Log Setting" to recorded the opration of "Add","Update" or "Delete" to log file.',SL_DOMAIN ) . '</li>';
		...
		$help = '<ol style="list-style-type:decimal;">';
			if (isset($patern[0]) && $patern[0]) {
				$help .= '<li>'.__('Button',SL_DOMAIN).
					'<br><img class="alignnone size-thumbnail wp-image-250" width="150" height="25" src="'.SL_PLUGIN_URL."images/".__('12_BUTTON.png',SL_DOMAIN).'">'.
			...
			if (isset($patern[2]) && $patern[2]) {
				$help .= '<li>'.__('Listing',SL_DOMAIN).'<br><img width="150" height="87" src="'.SL_PLUGIN_URL."images/".__('13_OPRATION.png',SL_DOMAIN).'">
<br>'.
	...
	public function help_side ($screen) {
		$screen->set_help_sidebar(
			...
			'<li>' . __( '<a href="https://salon.mallory.jp/en/?page_id=80" target="_blank">Documentation</a>',SL_DOMAIN ) . '</li>'.
...
//		wp_enqueue_script( 'dataTables', SL_PLUGIN_URL.'js/datatables.min.js',array( 'jquery' ) );
...
//		wp_enqueue_style('dataTables', SL_PLUGIN_URL.'css/dataTables.min.css');
...
// 			}
	...
	function _get_collation () {
		global $wpdb;

		$collate = $wpdb->collate;
		//usersとstaffをjoinする際に発生するエラー対処
		//usersのほうを優先する
		...
		if (empty($result[0]['Collation']) == false) {
			return " COLLATE ".$result[0]['Collation'];
		}
		return $collate;
	}
...
// 			$charset_collate .= " COLLATE $wpdb->collate";
		...
		$charset_collate = $this->_get_collation();
	...
	public function _update_spDate() {
		...
		if (count($result) > 0 ) {
			foreach ($result as $k1 => $d1 ) {
				...
				//$k2 = YYYY
				if (is_array($sp_dates)  && count($sp_dates) > 0 ) {

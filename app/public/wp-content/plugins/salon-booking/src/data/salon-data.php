...
//		if (empty($result['SALON_CONFIG_SEND_MAIL_TEXT_USER']) ) $result['SALON_CONFIG_SEND_MAIL_TEXT_USER'] = __('Mr/Ms {X-TO_NAME} Thank you for registration .your User_id is %s,your initial password is %s',SL_DOMAIN);
		...
		if (empty($result['SALON_CONFIG_USE_PC_MOBILE']) ) $result['SALON_CONFIG_USE_PC_MOBILE'] = Salon_Config::PC_MOBILE_USE;
 		...
 		if (!isset($result['SALON_CONFIG_CUSTOMER_DISPLAY_CNT']) ) $result['SALON_CONFIG_CUSTOMER_DISPLAY_CNT'] = Salon_Regist_Customer::DEFAULT_DISPLAY_CNT;
...
// 		return count($rows);
	...
	public function countNoWorkingStaffCount($branch_cd ,$in_time,$out_time){
	    global $wpdb;
	    $sql =
        ' SELECT  '.
        ' count(*) as staff_cnt ,sum(duplicate_cnt) as duplicate_cnt  '.
        ' FROM '.$wpdb->prefix.'salon_staff '.
        '   WHERE branch_cd = %d  '.
        '   AND   delete_flg <> %d '.
        '   AND  staff_cd in '.
        ...
        .'';
         ...
         $sql2 =
         '     AND (working_cds like "%'.Salon_Working::DAY_OFF.'%"'
         .'       OR working_cds like "%'.Salon_Working::LATE_IN.'%"'
         .'       OR working_cds like "%'.Salon_Working::EARLY_OUT.'%"))';

        $sql = $wpdb->prepare($sql	,$branch_cd,Salon_Reservation_Status::DELETED,
        ...
        return $result[0]['staff_cnt'] + $result[0]['duplicate_cnt'];
    }
...
// 				}
			...
			$status = sprintf(__('Action:%s Status:%s',SL_DOMAIN),__($type,SL_DOMAIN),$status);

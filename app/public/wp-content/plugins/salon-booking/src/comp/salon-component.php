...
class Salon_Component {
	...
	//		//で、実績として予約時の内容を設定しとく
			...
			if ( empty($d1['time_from_aft']) ) {
				...
				$result[$k1]['status_name'] = __('result is not registerd',SL_DOMAIN);
...
//							'     AND staff_cd = %d ';
					...
					if (+substr($result_branch[0]['close_time'],0,2) > 23 ) {
						...
						$sql  = $wpdb->prepare($sql,substr($set_data['time_from'],0,10),substr($set_data['time_to'],0,10)." 23:59:59",$set_data['staff_cd']);
					}
					if ($wpdb->query($sql) === false ) {
						$datas->_dbAccessAbnormalEnd();
					}
...
//			}
			...
			- $datas->countNoWorkingStaffCount($set_data['branch_cd'],$set_data['time_from'],$set_data['time_to']);
	...
	static function getMsg($err_cd, $add_char = '') {
		...
		switch ($err_cd) {
			...
			case 'E001':
				...
				break;
			case 'E002':
				$err_msg = sprintf(__("this user is not registerd",SL_DOMAIN));
				break;
			case 'E003':
				$err_msg = sprintf(__("this staff is not registerd[%s] ",SL_DOMAIN),$add_char);
				break;
			case 'E004':
			...
			case 'E005':
				...
				break;
			case 'E006':
				$err_msg = sprintf(__("user data is different %s",SL_DOMAIN),$add_char);
				break;
			case 'E007':
			...
			case 'E304':
				...
				break;
			case 'E305':
				$err_msg = __("\"Valid until\" need after today",SL_DOMAIN);
				break;
			case 'E401':

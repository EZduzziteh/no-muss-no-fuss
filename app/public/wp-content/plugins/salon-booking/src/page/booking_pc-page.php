...
?>
		$j(document).ready(function() {
			...
			scheduler.config.valid_from = new Date(<?php echo $this->insert_min_day_for_pc; ?>);
			scheduler.config.valid_to = new Date(<?php echo $this->insert_max_day_for_pc; ?>);
...
?>
	...
	<?php //休業日
			...
			//営業時間が00分でない場合（9:30～8:30)に、00分からずれている部分に色をつける
			parent::echoSetOutOfJustTime($this->branch_datas);
			//スタッフの設定
...
*/ ?>
				...
				if ( ev.start_date > new Date(<?php echo $this->insert_max_day_for_pc; ?>) ) {
					is_check = false;
...
?>
			...
			<?php //メニューがひとつ
				if (count($this->item_datas) == 1 ) : ?>
					$j("#sl_item_cds :<?php $this->echo_menu_type(); ?>").prop("checked",true);
					fnUpdateEndTime();
			<?php else: 	?>
...
?>
				...
				$j("#sl_item_cds input").prop("checked",false);
				if (ev.type) {
				...
				var max_loop = item_array.length;
				for	 (var i = 0 ; i < max_loop; i++) {
					$j("#sl_item_cds #sl_check_"+item_array[i]).prop("checked",true);
				}

...
// 		return Salon_Default::BRANCH_CD;
	...
	static function echoInitDataPaging($datas,$start_index,$total_records = 0) {
		$i = $start_index;
		...
		$jdata = array();
		$jdata['iTotalRecords'] = $total_records;
		$jdata['iTotalDisplayRecords'] = $total_records;
		if (empty($_REQUEST['sEcho']) == true) {
			$jdata['sEcho'] = 0;
		}
		else {
			$jdata['sEcho'] = @$_REQUEST['sEcho'];
		}
		if (is_null($datas) )$datas = array();
		$jdata['aaData'] = $datas;
		echo json_encode($jdata);
...
EOT;
	...
	static function echoDataTableLang($iDisplayLength = 100,$only_lang_display = false,$sEmptyTable = "",$sInfoEmpty = "") {
		...
		$sFirst = __('First',SL_DOMAIN);
		$sLast = __('Last',SL_DOMAIN);
		echo <<<EOT
			"bAutoWidth": false,
			"bProcessing": true,
			"sScrollX": "100%",
...
EOT2;
	...
	static function echoTableItem($items,$is_only_common_part = false,$is_multi_branch = true,$operate_width = '120px'
			,$isForceNoSort = false,$isForceNoSearch = false) {
...
EOT;
	...
	//				.attr("value",1)
	//				.prop("checked", aData.check == 1 ? true : false)
	//				.change(function() {
...
EOT2;
	...
	public function echoSetOutOfJustTime($branch_datas) {
	    $open_time_mm = +substr($branch_datas['open_time'],2,2);
	    $close_time_mm = +substr($branch_datas['close_time'],2,2);
	    //0分ジャストなら対処不要
	    if ($open_time_mm == 0 && $close_time_mm == 0) return;

	    $open_time_hh = +substr($branch_datas['open_time'],0,2);
	    $close_time_hh = +substr($branch_datas['close_time'],0,2);
	    $zones_top = ($open_time_hh*60).','.($open_time_hh*60+$open_time_mm);
	    $zones_bottom = ($close_time_hh*60 + $close_time_mm).','.(($close_time_hh+1)*60);

	    $set_days_detail_array = explode(';',$branch_datas['memo']);
	    //休み以外の日を設定する
	    for ($i = 0; $i < 7 ; $i++) {
	        $pos = strpos($branch_datas['closed'],strval($i));
	        if(  $pos === false){
	            echo <<<EOT
					var options = {
						days:{$i},
						zones:[{$zones_top}],
						type: "dhx_time_block",
						css: "dhx_sl_holiday dhx_sl_holiday_excepted",
						html: ""
					};
...
EOT;
	            ...
	            $pos = $this->getIndex(strval($i),$branch_datas['closed']);
	            $from_to = explode(',',$set_days_detail_array[$pos]);
	            if(+$branch_datas['open_time'] != +$from_to[0]
	                || +$branch_datas['close_time'] != +$from_to[1]){
	                echo <<<EOT2
					var options = {
						days:{$i},
						zones:[{$zones_top}],
...
EOT2;
	...
	private function getIndex ($target,$arrays){
        $splits = explode(',',$arrays);
        for($i = 0; $i < count($splits); $i++){
            if ($splits[$i] == $target) return $i;
        }
        return false;
	}
...
EOT2;
	...
	static function setItemContents() {
		...
		$item_contents['branch_name'] =array('id'=>'name'
		 ...
		 ,'tips' => __('within 10 charcters',SL_DOMAIN)
		...
		$item_contents['item_name'] =array('id'=>'name'
		 ...
		 ,'tips' => __('within 30 charcters',SL_DOMAIN)
		...
		$item_contents['short_name'] =array('id'=>'short_name'
		 ...
		 ,'tips' => __('within 5 charcters',SL_DOMAIN));
		...
		$item_contents['no_edit_remark'] =array('id'=>'remark'
		 ...
		 ,'tips' => __('within 300 charcters',SL_DOMAIN)
		 ,'table' => array(  'class'=>''
							,'width'=>''
							,'sort'=>'true'
							,'search'=>'true'
...
// 		 ,'tips' => __('Now plural shops select only.',SL_DOMAIN));
		...
		$item_contents['config_log'] =array('id'=>'config_is_log_need'
		 ,'class'	=>array()
		 ,'check' => array()
		 ,'label' => '2.'.__('Operation Log Setting',SL_DOMAIN)
		...
		$item_contents['config_show_detail_msg'] =array('id'=>'config_is_show_detail_msg'
		 ...
		 ,'tips' => __('when debug, check here',SL_DOMAIN));
		...
		$item_contents['send_mail_text'] =array('id'=>'send_mail_text'
		 ...
		 ,'tips' => __('within 500 charcters. {X-TO_NAME} in the content replace customer name',SL_DOMAIN));
		...
		$item_contents['send_mail_subject'] =array('id'=>'send_mail_subject'
		 ...
		 ,'tips' => __('within 78 charcters.',SL_DOMAIN));
		...
		$item_contents['holiday_display'] =array('id'=>'config_holiday_display'
				...
				,'tips' => __('If you need to replace charcters in the box of holiday, set this.',SL_DOMAIN));
		...
		$item_contents['special_onbusiness_display'] =array('id'=>'config_special_onbusiness_display'
				...
				,'tips' => __('If you need to replace charcters in the box of the shop on business day, set this.',SL_DOMAIN));
		$item_contents['onbusiness_display'] =array('id'=>'config_onbusiness_display'
				,'class'	=>array()
				,'check' => array()
				,'label' => __('Display \"Booking\"',SL_DOMAIN)
				,'tips' => __('If you need to replace charcters in the box that the staff registers as attendant, set this.',SL_DOMAIN));
		$item_contents['color_staff'] =array('id'=>'staff_color'
				,'class'	=>array()
				,'check' => array()
				,'label' => __('Staff color',SL_DOMAIN)
				,'tips' => __('Staff color on \"Reservation\"',SL_DOMAIN));
...
//								if( ! val.match(/^(?:\d{3}\-\d{4})$|^(?:\d{7})$/) ){
		...
		$check_contens['chkTel'] = '
							if ($j('.$target.').hasClass("chkTel") ) {
								if( ! val.match(/^[\d\-]{1,15}$/) ){
									item_errors.push( "'.__('within 15 charcters',SL_DOMAIN).'");
								}
							}';
			...
			for (var k1 in record) {
				if (record.hasOwnProperty(k1)){
					...
					case {$radio}:
						...
						\$j("#category_"+k1+"_option_wrap input").prop("checked",false);
						\$j("#category_"+k1+"_"+record[k1]).prop("checked",true);
						break;
					case {$check}:
						\$j("#category_"+k1+"_check_wrap input").prop("checked",false);
						var tmp_split = record[k1].split(",");
						for ( var i = 0 ; i < tmp_split.length ; i++ ) {
							\$j("#category_"+k1+"_"+tmp_split[i]).prop("checked",true);
						}
						break;
					}
				}

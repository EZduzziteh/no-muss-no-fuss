...
?>
			...
			$j("#working_cds input[type=checkbox]").prop("checked",false);
			...
			for	 (var i = 0 ; i<max_loop; i++) {
				$j("#working_cds #check_"+working_array[i]).prop("checked",true);
			}
			_setWorkingCds();
			$j("#name").focus();
		...
		function _setWorkingCds() {
			...
			if ($j("#check_<?php echo Salon_Working::DAY_OFF; ?>").is(":checked") ) {
				...
				$j("#check_<?php echo Salon_Working::HOLIDAY_WORK; ?>").prop("checked",false);
				$j("#check_<?php echo Salon_Working::USUALLY; ?>").prop("checked",false);
...
?>
			...
			if ($j("#check_<?php echo Salon_Working::EARLY_OUT; ?>").is(":checked") ||
				$j("#check_<?php echo Salon_Working::LATE_IN; ?>").is(":checked") ) {
				$j("#check_<?php echo Salon_Working::USUALLY; ?>").prop("checked",true);
			}
		}

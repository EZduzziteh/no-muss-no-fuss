...
//ここまで
			...
			target = $j("#lists").dataTable({
				...
				"fnDrawCallback": function () {
					<?php
						parent::echoEditableCommon("staff",array("ID","first_name","last_name"),'var setData = target.fnSettings();var position = target.fnGetPosition( td );if (!setData["aoData"][position[0]]["_aData"]["staff_cd"]) { alert("'.__('this user is not registerd',SL_DOMAIN).'"); return false; }');
					?>
				},
...
<?php //taregt_colはtdが前提 ?>
		function fnSelectRow(target_col) {
			...
			$j("#sl_tb_items_"+$j("#branch_cd").val()+" .sl_items_set").prop("checked",false);

			for (var i = 0; i < items_array.length; i++) {
				$j("#check_"+items_array[i]).prop("checked",true);
			}
		...
		<?php parent::echoDataTableEditColumn("staff","ID",$add_callback_process,'if (!target_cd) { alert("'.__('this user is not registerd',SL_DOMAIN).'"); return false; }'); ?>
		...
		function _checkEnableItems() {
			...
			$j(".sl_items_set").prop("checked",false);

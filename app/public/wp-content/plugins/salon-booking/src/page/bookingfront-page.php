...
class BookingFront_Page extends BookingCommon_Page {
	...
	private $insert_max_day_for_pc = '';
	private $insert_min_day_for_pc = '';
	...
	public function set_config_datas($config_datas) {
		...
		$this->insert_max_day_for_pc = substr($edit,0,4).','.(intval(substr($edit,5,2))-1).','.(intval(substr($edit,8,2)));

		$min = Salon_Component::computeDate(0);
		$this->insert_min_day_for_pc = substr($min,0,4).','.(intval(substr($min,5,2))-1).','.(intval(substr($min,8,2)));

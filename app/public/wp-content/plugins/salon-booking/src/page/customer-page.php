...
class Customer_Page extends Salon_Page {
	...
	private $customer_cnt = 0;
	private $display_cnt = Salon_Regist_Customer::DEFAULT_DISPLAY_CNT;
	private $is_on_servered_page = false;
...
// 		}
	...
	public function set_display_cnt($set_cnt = 0) {
		$this->customer_cnt = $set_cnt;
		if (Salon_Regist_Customer::MAX_GET_CNT < $set_cnt  ) {
			$this->is_on_servered_page = true;
			$this->display_cnt = $this->config_datas['SALON_CONFIG_CUSTOMER_DISPLAY_CNT'];
		}
	}
...
// 					parent::echoTableItem($seq,false,$this->is_multi_branch,'200px');
				?>
				<?php
					if ($this->is_on_servered_page ) {
						//検索対象を有効にするのは、最後の引数をfalse
						//あくまで、searchableがtrueになるだけなので
						parent::echoTableItem($seq,false,$this->isSalonAdmin(),'200px',true,false);
						echo '"bServerSide": true,';
						echo '"iDisplayLength" : '.$this->display_cnt.',';
...
//						echo '"sDom":"lrtip",';
				...
				"fnDrawCallback": function () {
					<?php parent::echoEditableCommon("customer",array("ID","first_name","last_name"),'var setData = target.fnSettings();var position = target.fnGetPosition( td );if (!setData["aoData"][position[0]]["_aData"]["customer_cd"]) { alert("'.__('this user is not registerd',SL_DOMAIN).'"); return false; }'); ?>
				},
...
<?php //	save_user_login_old = setData['aoData'][position[0]]['_aData']['user_login']; ?>
		...
		<?php parent::echoDataTableEditColumn("customer","ID","",'if (!target_cd) { alert("'.__('this user is not registerd',SL_DOMAIN).'"); return false; }'); ?>
...
<?php //							if ( (operate =="inserted")  && (save_user_login_old != $j("#user_login").val())) { ?>
				...
				if ($this->is_on_servered_page ) {
					...
					$first_name_position = 3;

					if ($this->config_datas['SALON_CONFIG_NAME_ORDER'] == Salon_Config::NAME_ORDER_OTHER ) {
						$last_name_position = 3;
						$first_name_position = 2;
					}
					echo <<<EOT
								target.fnUpdate( data.set_data['no'] ,parseInt(save_k1),0,false );
								target.fnUpdate( data.set_data['last_name'] ,parseInt(save_k1),{$last_name_position},false );
								target.fnUpdate( data.set_data['first_name'] ,parseInt(save_k1),{$first_name_position},false );
								target.fnUpdate( data.set_data['remark'] ,parseInt(save_k1),5,false );
								target.fnUpdate( data.set_data['branch_name'] ,parseInt(save_k1),6,false );
								set_update_data( setData['aoData'][save_k1]['_aData'],data.set_data );
EOT;
				}
				else {
					echo 'target.fnUpdate( data.set_data ,parseInt(save_k1) );';
				}
				?>
							}
...
if ($this->is_on_servered_page ) {
	echo <<<EOT
		function set_update_data (origin, result) {
			origin['address'] = result['address'];
			origin['branch_cd'] = result['branch_cd'];
			origin['branch_name'] = result['branch_name'];
			origin['first_name'] = result['first_name'];
			origin['last_name'] = result['last_name'];
			origin['mail'] = result['mail'];
			origin['mobile'] = result['mobile'];
			origin['rank_patern_cd'] = result['rank_patern_cd'];
			origin['remark'] = result['remark'];
			origin['tel'] = result['tel'];
			origin['zip'] = result['zip'];
		}
EOT;
}
?>

			...
			?>
				...
				<?php //メニューがひとつ
					if (count($this->item_datas) == 1 ) {
						echo '$j("#sl_item_cds :'.$this->set_menu_type.'").prop("checked",true);';
					}
				?>
...
<?php	endif; ?>
			...
			$j("#sl_item_cds input[type=<?php $this->echo_menu_type(); ?>]").prop("checked",false);
...
?>
								...
								for	 (var i = 0 ,max_loop = item_array.length; i < max_loop; i++) {
									$j("#slm_chk_"+item_array[i]).prop("checked",true);
								}
